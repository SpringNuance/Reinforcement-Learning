...
class Policy(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        # constant standard deviation σ = 1 (i.e. log(σ) = 0) 
        # For Task 1a, 1b, 1c
...
class PG(object):
    ...
    def update(self,):
        ...
        # # Task 1b
        # baseline = 20
        # discounted_rewards = h.discount_rewards(rewards, self.gamma) - baseline

        # # Task 1c and Task 2
        # baseline = 0
        # jitter = 1e-12
        # discounted_rewards = h.discount_rewards(rewards, self.gamma) - baseline
        # discounted_rewards = (discounted_rewards - torch.mean(discounted_rewards)) / (torch.std(discounted_rewards) + jitter)
    ...
    def get_action(self, observation, evaluation=False):
        ...
        act_logprob = distribution.log_prob(action).sum(-1)
