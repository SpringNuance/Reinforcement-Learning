...
class SailingGridworld(object):
    ...
    def __init__(self, rock_penalty=-2, harbour_reward=10, wind_p=0.1, value_update_iter=100):
        ...
        self.episode_steps = 0 # to record steps of current episode
        self.max_episode_steps = 100 # will force to terminate once beyond 100 steps
    ...
    def step(self, action):
        ...
        # Terminate the crrent episode once beyond self.max_episode_steps
    ...
    def record_values_policy(self, values, policy, filename='value.gif'):
        self.clear_text()
    ...
    def save_values_policy(self, filename='value.gif'):
        frames = np.stack(self.value_frames)
        imageio.mimsave(filename, frames, format = 'GIF-PIL', fps = 1)
    
    # render env
    def render(self, filename='env.gif'):
        img = self._render()
        ...
        if self.episode_finished: 
            ...
            imageio.mimsave(filename, frames, format = 'GIF-PIL', fps = 5) 
